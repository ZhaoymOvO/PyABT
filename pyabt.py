...
has_sox = re.findall(r'[sS][oO][xX]', os.getenv('PATH'))
argv = sys.argv[1:]
...
target_file_list = [f"{re.findall(r'(.+)\..+?', i)[0]}.{target_format}" for i in source_file_list]

for i in range(len(source_file_list)):
    if os.name == 'nt':
        os.system(f"sox '{source_path}\\{source_file_list[i]}' '{target_path}\\{target_file_list[i]}'")
    else:
        os.system(f"sox '{source_path}/{source_file_list[i]}' '{target_path}/{target_file_list[i]}'")
